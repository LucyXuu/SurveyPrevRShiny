...
mod_indicator_in_app_server <- function(id,CountryInfo,AnalysisInfo,parent_session){
  moduleServer( id, function(input, output, session){
    ...
    output$selected_ind_tab <- DT::renderDataTable({
      ...
      selected_ind_table <- ref_tab_mics
      ...
      colnames(selected_ind_table) <- c("Standard ID" ,"Label","Full Definition","MICS Report Chapter")
      print(names(selected_ind_table))
      print(names(DHS_ind_dictionary))
      
      selected_ind_table <- merge(selected_ind_table[, c("Standard ID" ,"Label","MICS Report Chapter")],
      ...
      selected_ind_table <- selected_ind_table[,c("Standard ID" ,"Label","Full Definition","MICS Report Chapter",
    ...
    output$text_app_ind <- renderUI({
      ...
      HTML(paste0(
        ...
        "<p style='font-size: large;'>",
        "The app currently supports analysis of four indicators, all sourced from the official MICS report. ",
    ...
    output$DHS_ind_supported_tab <- DT::renderDataTable({
      ...
      colnames(ind_in_app) <- c("Standard ID" ,"Label","Full Definition","MICS Report Chapter")
