      ...
      content = function(file) {
        ...
        
        {
          ...
          tryCatch({
            ...
            grid.text("Survey Source: ", x = 0.05, y = 0.75, just = "left", gp = gpar(fontsize = 12))
            grid.text("MICS Survey", x = 0.2, y = 0.75, just = "left", gp = gpar(fontsize = 12, fontface = "bold"))
            
            grid.text("Indicator: ", x = 0.05, y = 0.7, just = "left", gp = gpar(fontsize = 12))
            grid.text(CountryInfo$svy_indicator_des(), x = 0.2, y = 0.7, just = "left", gp = gpar(fontsize = 12, fontface = "bold"))
            
            grid.text("Levels: ", x = 0.05, y = 0.65, just = "left", gp = gpar(fontsize = 12))
            grid.text(concatenate_vector_with_and(CountryInfo$GADM_analysis_levels()), x = 0.2, y = 0.65, just = "left", gp = gpar(fontsize = 12, fontface = "bold"))
            
            # Title for number of regions section
            grid.text("Number of regions at selected admin levels:", x = 0.05, y = 0.55, just = "left", gp = gpar(fontsize = 12))
            
            # Table for number of admin regions (placed lower)
            pushViewport(viewport(y = 0.55, height = unit(1.5, "inches"), just = "top"))
            ...
            grid.text("Detailed information on the indicator:", x = 0.05, y = 0.55, just = "left", gp = gpar(fontsize = 12))
            upViewport()
            ...
            #     ind_detailed_tab <<- ref_tab_mics[ref_tab_mics$ID == tmp_indicator_ID, c('ID', 'Topic', 'Full_definition')]
            #   } else {
            ...
            colnames(ind_detailed_tab) <- c('Standard ID', 'MICS Report Chapter', 'Definition')
            ...
            ind_detailed_tab$`MICS Report Chapter` <- sapply(ind_detailed_tab$`Report Chapter`, wrap_text, width = 50)
            ...
            grid.text(paste0("*"), x = 0.05, y = 0.1, gp = gpar(fontsize = 12, col = "red"),
                      just = "left")
            ...
            grid.text(paste0("For indicators with multiple versions, the app defaults to the ",
                      ...
                      x = 0.06, y = 0.1, just = "left", gp = gpar(fontsize = 12))
            
            grid.text(paste0("and unstratified age groups (total)."),
                      x = 0.05, y = 0.07, just = "left", gp = gpar(fontsize = 12))
            ...
            if(CountryInfo$MICS_version()) {
              ...
              tryCatch({
                ...
                ind_api_est <- DHS_api_est[DHS_api_est$`Country Code`==DHS_country_code&
                                             DHS_api_est$`Standard ID`==tmp_indicator_ID&
                ...
                if(dim(ind_api_est)[1]==0){
                  ...
                  ind_api_est <- ind_api_est[,c('Country','Survey Year',
                                                'Standard ID','Full Definition','Estimate','By Variable Label')]
          ...
          tryCatch({
            ...
            natl_ci_lower = tmp.natl.res$direct.lower
            natl_ci_upper = tmp.natl.res$direct.upper
            
            natl_est <- format(round(natl_est, digits=2), nsmall = 2)
            if(grepl("dying", CountryInfo$svy_indicator_des())|grepl("ortality", CountryInfo$svy_indicator_des())){
              natl_ci <- paste0("(", format(round(natl_ci_lower, digits=5), nsmall = 5),", ", format(round(natl_ci_upper, digits=5), nsmall = 5),")")
            }else{
              natl_ci <- paste0("(", format(round(natl_ci_lower, digits=3), nsmall = 3),", ", format(round(natl_ci_upper, digits=3), nsmall = 3),")")
            }
            ...
            grid.text("National estimate: ", x = 0.05, y = 0.40, just = "left", gp = gpar(fontsize = 12))
            grid.text(paste0(natl_est,description_suffix),
                      x = 0.3, y = 0.40, just = "left", gp = gpar(fontsize = 12, fontface = "bold"))
            
            grid.text("95% Confidence Interval: ", x = 0.05, y = 0.30, just = "left", gp = gpar(fontsize = 12))
            grid.text(natl_ci,
                      x = 0.3, y = 0.30, just = "left", gp = gpar(fontsize = 12, fontface = "bold"))
